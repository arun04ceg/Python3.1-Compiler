    ...
    (define $i (gensym 'i))
     ...
     `(let ([,$b ,expr])
        (let ([,$i ,(transform-expr env i)])
          (cond
            [(tuple? ,$b)    (tuple-set! ,$b ,$i ,(transform-expr env $base))]
            [(py-list? ,$b)  (py-list-set! ,$b ,$i ,(transform-expr env $base))]
            [(dict? ,$b)     (dict-set! ,$b ,$i ,(transform-expr env $base))])))]
...
(define (set-index-augassign env $base index op expr)
  (match index
    [`(subscript ,i)
     ;`(unfinished-augassign)]
    (define $i (gensym 'i))
     (define $b (gensym 'b))
      (define $v (gensym 'v))
     `(let ([,$b ,expr])
        (let ([,$i ,(transform-expr env i)])
         (let ([,$v ,(unwind-augassign-trailer env $b `(subscript ,$i))])
          (cond
            [(tuple? ,$b)    (tuple-set! ,$b ,$i (,op ,$v ,(transform-expr env $base)))]
            [(py-list? ,$b)  (py-list-set! ,$b ,$i (,op ,$v ,(transform-expr env $base)))]
            [(dict? ,$b)     (dict-set! ,$b ,$i (,op ,$v ,(transform-expr env $base)))]))))]
...
(define (unwind-var env var) (if (set-member? env var) `(set! ,var) `(set-global! ,var)))
...
(define (transform-stmt env stmt)
  (match stmt
    ...
    [`(= ((indexed ,base ,trailers ... ,index)) ,value)
     ; =>
     (set-index env value index (unwind-trailers env (transform-expr env base) trailers))]
    
    [`(= ,(and lvals `(,_ ,_ . ,_)) ,expr)
     ; =>
     (define $t (gensym 't))
     ;`(let ([,$t ,(transform-expr env expr)])
     ; ,(map (lambda (ele) (transform-stmt env `(= ,ele ,(transform-expr $t)))) lvals))]
     (define $ilist (gensym 'ilist))
     (define $elist (gensym 'elist))
    `(let ([,$t ,(transform-expr env expr)])
     ,@(let 
          ([$ilist (map (lambda (val) (gensym 'i)) lvals)]
          [$elist (map (lambda (val) (gensym 'e)) lvals)])
            (map (lambda (ele i)
             `(,@(unwind-var env ele)
             (let ([,(list-ref $elist i) ,$t])
                (let ([,(list-ref $ilist i) ,(transform-expr env i)])
                (cond
                   [(py-list? ,(list-ref $elist i))  (py-list-ref ,(list-ref $elist i) ,(list-ref $ilist i))]
                   [(tuple? ,(list-ref $elist i))    (tuple-ref ,(list-ref $elist i) ,(list-ref $ilist i))]
                   [(dict? ,(list-ref $elist i))     (dict-ref ,(list-ref $elist i) ,(list-ref $ilist i))]
                   [else            (error "cannot index object")])))))lvals (sequence->list (in-range 0 (length lvals))))))]
    ...
    [`(,(and aug-op (augassign)) (,(and (? symbol?) lval)) ,expr)
     ...
     `(,@(unwind-var env lval) ,result)]
    
    
    [`(,(and aug-op (augassign)) ((indexed ,base ,trailers ... ,index)) ,value)
     ; =>
     ;`(unfinished-transform-stmt-4)]
     (define op (select-augassign aug-op))
     (set-index-augassign env value index op (unwind-trailers env (transform-expr env base) trailers))]
    ...
    [`(assert ,expr1 ,expr2)
     ; =>
     `(assert ,(transform-expr env expr1) ,(transform-expr env expr2))]
    ...
    [`(cond [,tests ,suites] ... [else ,otherwise])
     ; =>
     `(cond ,@(map (lambda (t s) (list (transform-expr env t) 
				       (transform-suite env s))) tests suites) (else ,(transform-suite env otherwise)))]
    ...
    [`(for ,var ,seq ,suite)
     ; =>
     (define $i (gensym 'i))
     `(for-each ,$i ,(transform-expr env seq) (begin (,@(unwind-var env var) ,$i) ,(transform-suite env suite)))]
     
    [`(for ,var ,seq ,suite ,else-suite)
     ; =>
     (define $i (gensym 'i))
     `(for-each ,$i ,(transform-expr env seq) (begin (,@(unwind-var env var) ,$i) ,(transform-suite env suite) ,(transform-suite env else-suite)))]
...
(define (unwind-comparison env expr ops)
  (match ops
    ...
    [(cons (list cmp rhs) rest)
     ...
     `(let ([,$cv ,(transform-expr env rhs)]) (if (,(select-cmp cmp) ,expr ,$cv) ,(unwind-comparison env $cv rest) #f))]
...
(define (unwind-trailer env $expr trailer)
  (match trailer
    ...
    [`(called . ,args)
      (cons $expr (unwind-args env args))]
...
(define (unwind-augassign-trailer env $expr trailer)
  (match trailer
    [`(dot ,NAME)
     `(get-field ,$expr ,NAME)]
...
(define (unwind-args env args)
           (match args
              [`() '()]
              [(cons arg rest) (cons (transform-expr env arg) (unwind-args env rest))]))   
...
(define (transform-expr env expr)
  (match expr
    ...
    [`(indexed ,expr . ,trailers)
     ...
     ;(cons (transform-expr env expr) (unwind-trailers env (transform-expr env expr) trailers))]
    
    [`(power ,base ,expn)
     `(expt ,(transform-expr env base) ,(transform-expr env expn))]
    
    [`(set . ,exprs)
     `(set ,@(unwind-args env exprs))]
    
    [`(tuple . ,exprs)
     `(tuple ,@(unwind-args env exprs))]
    
    [`(list . ,exprs)
     `(py-list* ,@(unwind-args env exprs))]
    
    [`(dict . ,pairs)
     `(dict ,@(map (lambda (t) (list (transform-expr env (car t)) (transform-expr env (cadr t)))) pairs))]
